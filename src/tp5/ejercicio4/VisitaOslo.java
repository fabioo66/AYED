...
public class VisitaOslo {
	...
	public List<String> paseoEnBici(Graph<String> lugares, String destino, int maxTiempo, List<String> lugaresRestringidos){
		...
		if (origen != null && dest != null) {
			dfs(lugares, origen, dest, new boolean [lugares.getSize()], maxTiempo, lugaresRestringidos, camino);
		}
		
		return camino;
	}
	
	private boolean dfs(Graph<String> lugares, Vertex<String> origen, Vertex<String> destino, boolean [] visitados, int maxTiempo, List<String> lugaresRestringidos, List<String> camino) {
		boolean ok = false;
		...
		if (origen == destino) {
			...
			while(iter.hasNext() && !ok) {
				...
				if (!visitados[j] && maxTiempo - vertex.getWeight() >= 0 && !lugaresRestringidos.contains(vertex.getTarget().getData())) {
					ok = dfs(lugares, vertex.getTarget(), destino, visitados, maxTiempo - vertex.getWeight(), lugaresRestringidos, camino);
				}
			}
		}
		if (!ok) {
