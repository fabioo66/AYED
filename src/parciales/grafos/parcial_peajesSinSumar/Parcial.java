package parciales.grafos.parcial_peajesSinSumar;
    ...
    public List<String> caminoConPresupuesto(Graph<String> ciudades, String origen, String destino, int montoMaximo){
        ...
        if (!ciudades.isEmpty()){
            ...
            if (ori != null && desti != null){
                dfs(ciudades, ori, desti, camino, new boolean [ciudades.getSize()], montoMaximo);
            }
        }
        return camino;
    }

    public boolean dfs(Graph<String> ciudades, Vertex<String> origen, Vertex<String> destino, List<String> camino, boolean [] visitados, int montoMaximo){
        boolean ok = false;
        ...
        if (origen.getData().equals(destino.getData())){
            ...
            while (iter.hasNext() && !ok){
                ...
                if (!visitados[vertice.getPosition()] && montoMaximo - arista.getWeight() >= 0){
                    ok = dfs(ciudades, vertice, destino, camino, visitados, montoMaximo - arista.getWeight());
                }
            }
        }
        if (!ok){
